...
def test_book_spread_zs_iex_example() -> None:
    # A stale low bid cannot be hidden by a nearby last trade.
...
def test_iex_outside_book_uses_book() -> None:
    q = _quote(72.0, 72.8)
    bps = spread_bps_for_entry(q, last_price=71.5, feed="sip")
...
def test_iex_last_above_ask_does_not_zero_spread() -> None:
    ...
    bps = spread_bps_for_entry(q, last_price=162.23, feed="sip")
    assert bps == pytest.approx(book_spread_bps(q))


def test_iex_orphan_ask_does_not_zero_spread() -> None:
    # A stale low bid and high ask still expose the full live-book width.
...
def test_axp_screenshot_wide_quote_cannot_pass_via_tape(last):
    ...
    assert spread_bps_for_entry(q, last_price=last, feed="sip") == pytest.approx(75.9933, rel=1e-5)
...
def test_invalid_book_is_unavailable(bid, ask):
    ...
    assert spread_bps_for_entry(q, last_price=100, feed="sip") is None
