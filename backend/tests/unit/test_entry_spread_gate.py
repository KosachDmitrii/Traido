...
def test_all_alpaca_environments_default_to_sip(monkeypatch) -> None:
    from core.config import Settings

    paper = Settings(TRAIDO_BROKER_ENV="paper")
    live = Settings(TRAIDO_BROKER_ENV="live")
...
def test_non_sip_configuration_is_rejected(monkeypatch) -> None:
    ...
    with pytest.raises(ValidationError):
        Settings(TRAIDO_BROKER_ENV="paper", ALPACA_DATA_FEED="iex")
