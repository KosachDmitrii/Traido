...
def test_stream_requires_all_minutes_and_applies_late_corrections():
    ...
    start = p.range_end
    for i in [0, 1, 3, 4]:
        assert alpaca_stream.ingest(minute(i, start), now=now)
    assert load_bars("alpaca:sip", p.symbol, start, now) == []
    assert alpaca_stream.ingest(minute(2, start), now=now)
    bars = load_bars("alpaca:sip", p.symbol, start, now)
    assert len(bars) == 1 and bars[0].volume == 50 and bars[0].timeframe == Timeframe.M5
    update = minute(4, start, 102)
    update["T"] = "u"
    alpaca_stream.ingest(update, now=now)
    assert load_bars("alpaca:sip", p.symbol, start, now)[0].close == 102


def test_stream_rejects_unclosed_or_invalid_source_minute():
    p, _, now = scenario()
    assert not alpaca_stream.ingest(minute(0, now), now=now)
    bad = minute(0, p.range_end)
    bad["h"] = 50
    assert not alpaca_stream.ingest(bad, now=now)
    assert load_bars("alpaca:sip", p.symbol, p.range_end, now) == []
...
async def test_complete_live_stream_survives_rest_outage_but_stale_stream_cannot_authorize(
    ...
    p, rows, now = scenario()
    p = p.model_copy(update={"source": "alpaca:sip"})
    save_bars(p.source, p.symbol, rows)
    ...
    alpaca_stream._latest[p.symbol] = now - timedelta(minutes=2)
    with pytest.raises(TimeoutError):
