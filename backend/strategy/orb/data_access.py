"""Observable SIP access, checked even while the exchange is closed."""
...
def data_error_reason(exc: Exception) -> str:
...
async def check_access(market_data: Any) -> dict[str, Any]:
    ...
    if getattr(market_data, "_feed", None) != "sip":
        ...
        try:
            ...
            reason = data_error_reason(exc)
    if DATA_ACCESS.get("reason") != reason or DATA_ACCESS.get("status") == "unchecked":
        logging.getLogger(__name__).warning("ORB data access: %s", reason or "accessible")
    DATA_ACCESS.update(
