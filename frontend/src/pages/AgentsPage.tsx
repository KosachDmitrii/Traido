...
const AGENT_META: Record<string, AgentMeta> = {
  ...
  context: {
    icon: Globe2,
    blurbKey: "agents.meta.context.blurb",
    accent: "taupe",
    tipKey: "agents.meta.context.tip",
    feedsFrom: ["scanner"],
    feedsTo: ["universe"],
  },
  universe: {
    icon: Radar,
    blurbKey: "agents.meta.universe.blurb",
    accent: "taupe",
    tipKey: "agents.meta.universe.tip",
    feedsFrom: ["context"],
    feedsTo: ["structure"],
  },
  structure: {
    icon: CandlestickChart,
    blurbKey: "agents.meta.structure.blurb",
    accent: "taupe",
    tipKey: "agents.meta.structure.tip",
    feedsFrom: ["universe"],
    feedsTo: ["setup"],
  },
  setup: {
    icon: Compass,
    blurbKey: "agents.meta.setup.blurb",
    accent: "mustard",
    tipKey: "agents.meta.setup.tip",
    feedsFrom: ["structure"],
    feedsTo: ["entry"],
  },
  entry: {
    icon: CandlestickChart,
    blurbKey: "agents.meta.entry.blurb",
    accent: "mustard",
    tipKey: "agents.meta.entry.tip",
    feedsFrom: ["setup"],
    feedsTo: ["risk_plan"],
  },
  risk_plan: {
    icon: ShieldCheck,
    blurbKey: "agents.meta.risk_plan.blurb",
    accent: "ink",
    tipKey: "agents.meta.risk_plan.tip",
    feedsFrom: ["entry"],
    feedsTo: ["checklist"],
  },
  checklist: {
    icon: Newspaper,
    blurbKey: "agents.meta.checklist.blurb",
    accent: "sage",
    tipKey: "agents.meta.checklist.tip",
    feedsFrom: ["risk_plan"],
    feedsTo: ["risk"],
  },
  risk: {
...
const LANES: { id: string; labelKey: MessageKey; agentIds: string[] }[] = [
  ...
  {
    id: "analyze",
    labelKey: "agents.lane.analyze",
    agentIds: ["context", "universe", "structure", "setup"],
  },
  {
    id: "decide",
    labelKey: "agents.lane.decide",
    agentIds: ["entry", "risk_plan", "checklist", "risk"],
  },
    ...
    <div className="ag-page">
      <section className="ag-rail" aria-label={t("agents.pipeline.aria")}>
        {LANES.map((lane, i) => {
          ...
          const hot = laneAgents.some((a) => agentDisplayStatus(a) === "working");
          return (
