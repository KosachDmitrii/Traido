...
export function EvaluationPage() {
  ...
  return <div className={styles.page}>
    ...
    <section className={styles.card}><div className={styles.sectionHead}><h2>{ru ? "Планы и состояния" : "Plans & states"}</h2><span>{plans.length}</span></div>
      {!plans.length ? <p className={styles.description}>{ru ? "Планы появятся после отбора. Отсутствие планов не означает убыточную стратегию." : "Plans appear after selection. No plans does not imply a losing strategy."}</p> : <div className={styles.scroll}><table><thead><tr>{(ru ? ["Тикер","Начало входа","Стоп","Макс. вход","Отн. объём","Состояние"] : ["Symbol","Trigger","Stop","Max entry","Rel. volume","State"]).map(label => <th key={label}>{label}</th>)}</tr></thead><tbody>{pager.slice.map(plan => { const forming = ["orb@2.0.0", "orb@2.1.0", "orb@2.2.0"].includes(plan.version ?? "") && !plan.evidence?.retest; return <tr key={plan.symbol}><td><strong>{plan.symbol}</strong></td><td>{px(forming ? null : plan.trigger)}</td><td>{px(forming ? null : plan.stop)}</td><td>{px(forming ? null : plan.max_entry)}</td><td>{px(plan.relative_volume)}×</td><td><span className={styles.status}>{orbState(orb?.states?.[plan.symbol]?.state)}</span></td></tr>; })}</tbody></table></div>}
