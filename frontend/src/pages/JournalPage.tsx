...
export function JournalPage() {
  ...
  return <div className={styles.page}>
    ...
    <section className={styles.history} aria-busy={loading}>
      {!!data?.unverified_count && <p role="status">{ru ? `Цена выхода не подтверждена у ${data.unverified_count} сделок. Они исключены из статистики до восстановления исполнений брокера.` : `${data.unverified_count} trades have unverified exit prices and are excluded from statistics until broker fills are recovered.`}</p>}
      <p>{ru ? "P&L рассчитан по ценам исполнения до отдельных комиссий и сборов брокера." : "P&L uses execution prices before separate broker fees and charges."}</p>
      ...
      {loading || error || !data?.items.length ? <div className={styles.empty} role="status"><BookOpen size={28} aria-hidden /><h3>{loading ? (ru ? "Загружаем сделки…" : "Loading trades…") : error ? (ru ? "Не удалось загрузить журнал" : "Could not load the journal") : t("journal.empty")}</h3>{error && <Button variant="ghost" onClick={() => setRetry(n => n + 1)}>{ru ? "Повторить" : "Retry"}</Button>}</div> : <div className={styles.scroll} tabIndex={0} role="region" aria-label={ru ? "История сделок" : "Trade history"}><table><thead><tr>
        ...
        return <tr key={trade.id ?? i}><td><div className={styles.identity}><span className={styles.symbolIcon}>{trade.symbol.slice(0,2)}</span><div><strong>{trade.symbol}</strong>{trade.name && <small>{trade.name}</small>}</div></div></td>
          ...
          <td>{price(trade.entry)}</td><td>{trade.exit_price_status === "unverified" ? (ru ? "Не подтверждена" : "Unverified") : price(trade.exit)}</td><td><span className={styles.pnl} data-tone={pnl === null || pnl === 0 ? "neutral" : pnl > 0 ? "positive" : "negative"}>{price(trade.pnl, true)}</span></td><td>{numeric(trade.pnl_pct) === null ? "—" : `${price(trade.pnl_pct, true)}%`}</td><td><span className={styles.strategy}>{trade.strategy_version || "—"}</span></td></tr>;
      })}</tbody></table></div>}
