...
export function OpportunityRail({ desk, onFlash, onRefresh, layout = "rail" }: Props) {
  ...
  const planPriority = (symbol: string) => {
    const opp = buys.find(o => o.candidate.symbol === symbol && ["orb@1.1.0", "orb@1.2.0", "orb@1.3.0", "orb@1.4.0", "orb@1.5.0", "orb@2.0.0", "orb@2.1.0"].includes(o.candidate.strategy_version ?? ""));
    if (!opp) return 2;
  ...
  return <section className={layout === "page" ? styles.page : styles.rail}>
    ...
    {plans.map(plan => {
      ...
      const isRetest = ["orb@2.0.0", "orb@2.1.0"].includes(plan.version ?? "");
      const ready = !isRetest || retest?.phase === "ready";
      const opp = buys.find(o => o.candidate.symbol === plan.symbol && ["orb@1.1.0", "orb@1.2.0", "orb@1.3.0", "orb@1.4.0", "orb@1.5.0", "orb@2.0.0", "orb@2.1.0"].includes(o.candidate.strategy_version ?? ""));
